...
object::update(void)
{
//    this->m_node->get_transform().rotate(0.5f, QVector3D(0.0, 1.0, 0.0));
}
...
object::render(const QMatrix4x4 &projection, const QMatrix4x4 &view, const QMatrix4x4 &model)
{
    ...
    if (this->m_mesh != 0) {
        ...
        this->m_skin_shader_program->setUniformValue("lights[0].position", QVector3D(5.0, 0.0, 0.0));
        this->m_skin_shader_program->setUniformValue("lights[1].position", QVector3D(-5.0, 0.0, 0.0));
